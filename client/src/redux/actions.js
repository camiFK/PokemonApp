...
export const getPokemons = () => dispatch => {
    return axios(`https://cfkpokeapp.herokuapp.com/pokemons`)
    .then((json) => dispatch({type: 'GET_POKEMONS', payload: json.data}))
    .catch(error => console.log(error))
}

export const getPokeDetail = (id) => dispatch => {
    return axios(`https://cfkpokeapp.herokuapp.com/pokemons/${id}`)
...
export function getPokeName(name) {
    return async function(dispatch) {
        try {
            const json = await axios(`https://cfkpokeapp.herokuapp.com/pokemons?name=${name}`)
...
export function getAllTypes() {
    return async function(dispatch) {
        try {
            const json = await axios.get(`https://cfkpokeapp.herokuapp.com/types`)
 ...
 export function postPokemon(input) {
     return async function(dispatch) {
         try {
             let newPokemon = await axios.post('https://cfkpokeapp.herokuapp.com/pokemons', input)
